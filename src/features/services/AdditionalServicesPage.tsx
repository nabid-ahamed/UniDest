...
export default function AdditionalServicesPage() {
  // Server-backed: the list, the status vocabulary and the assignable staff all
  // come from the API, so this screen and the portal cannot disagree.
  ...
  const [assignReq, setAssignReq] = useState<ApiServiceRequest | null>(null)
  const [deleteReq, setDeleteReq] = useState<ApiServiceRequest | null>(null)
  ...
  }, [rows, search, service, status, country, assigned, createdDate])
    ...
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            {pageRows.length === 0 && (
              <tr>
                <td colSpan={9} className="px-4 py-12 text-center text-sm text-slate-500">
                  {isPending ? 'Loading service requests…' : 'No service requests found.'}
                  {activeFilterCount > 0 && (
              ...
              <div className="grid grid-cols-1 gap-3 px-6 py-5 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Field label="Country">
                  <select value={country} onChange={(e) => { setCountry(e.target.value); setPage(1) }} className="input">
                    ...
                    {[...new Set([...allCountries, ...rows.map((r) => r.country)])].filter(Boolean).sort().map((c) => (
      ...
      {assignReq && (
        <AssignStaffDialog
          ...
          staff={staffOptions.map((s) => s.name)}
          onClose={() => setAssignReq(null)}
          onSave={(member) => {
            updateRequest.mutate(
              { id: assignReq.id, assignedTo: member },
              { onSuccess: () => showToast(`Request #${assignReq.id} assigned to ${member}`) },
            )
            setAssignReq(null)
          }}
        />
          ...
          <ConfirmDialog
            ...
            message={
              <>
                ...
                {deleteReq.service} — {deleteReq.studentName}) will be removed from this list.
              </>
            }
            confirmLabel="Delete"
            onConfirm={() => {
              deleteRequest.mutate(deleteReq.id, {
                onSuccess: () => showSuccessDialog(`Request #${deleteReq.id} deleted successfully`),
              })
              setDeleteReq(null)
            }}
